...
(async () => {
	...
	for await (let policy of driver.listPolicies()) {
		// approving the policy on behalf of the insurer
		// and refreshing the model to get the latest data
		await policy.internalApprove({ refresh: true, approvedById: "<< my user id (optional) >>" });

		console.assert(policy.isApproved() === true);
	}
})();
